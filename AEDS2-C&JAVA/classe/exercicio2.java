...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 5; i++) p[i] = new Produto();
        double media = 0;
        for (int i = 0; i < 5; i++) p[i].preenche(sc);
        for (int i = 0; i < 5; i++) p[i].exibe();
        for (int i = 0; i < 5; i++) {
            System.out.println("Preço final do produto " + (i + 1) + ": " + p[i].getFinal() + " reais");
            media += p[i].getFinal();
        }
        media /= 5;
        System.out.println("Média de preço dos produtos: " + media + " reais!");
        for (int i = 0; i < 5; i++) {
            if (p[i].getFinal() > media) System.out.println("Produto " + (i + 1) + " tem preço acima da média!");
